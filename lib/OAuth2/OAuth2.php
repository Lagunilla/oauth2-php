...
class OAuth2 {
   ...
   */
  public function grantAccessToken(Request $request = NULL, $authCode = NULL) {
      //@Modified - Nos pasan el objeto $authCode
    $filters = array(
    ...
    switch ($input["grant_type"]) {
      case self::GRANT_TYPE_AUTH_CODE:
        //@Modified - SI nos han pasado el objeto authCode, lo utilizamos, en lugar de hacer una busqueda en la base de datos
        if ($authCode == NULL) {
            $stored = $this->grantAccessTokenAuthCode($client, $input, $request); // returns array('data' => data, 'scope' => scope)
        } else {
            $stored = array(
  ...
  protected function grantAccessTokenAuthCode(IOAuth2Client $client, array $input, $request = NULL) {
    ...
    $authCode = $this->storage->getAuthCode($input["code"], $client, $request);
   ...
   */
  protected function getAuthorizeParams(Request $request = NULL, $client = NULL) {
    $filters = array(
    ...
    if ($client == NULL) {
        ...
        $client = $this->storage->getClientById($client->getId());
    }
    if (! $client) {
   ...
   * @ingroup oauth2_section_4
   */
    public function finishClientAuthorization($is_authorized, $data = NULL, Request $request = NULL, $scope = NULL, $client = NULL) {

    // In theory, this could be POSTed by a 3rd-party (because we are not
    // internally enforcing NONCEs, etc)
    $params = $this->getAuthorizeParams($request, $client);
    ...
    if ($is_authorized === FALSE) {
      ...
      if ($params["response_type"] == self::RESPONSE_TYPE_AUTH_CODE) {
        $codeObject = $this->createAuthCode($params["client"], $data, $params["redirect_uri"], $scope);
        $result["query"]["code"] = $codeObject->getToken();
        //@Modified guardamos el objeto en otro campo
        $result["query"]["codeObject"] = $codeObject;
      }
    ...
    //@Modified En lugar de generar la url (no se usa la url), devolvemos el objeto que nos han devuelto
    return $result;
  ...
  private function createAuthCode(IOAuth2Client $client, $data, $redirect_uri, $scope = NULL) {
    ...
    //@Modified - En lugar de devolver el código, deveolvemos el objeto entero
    $codeObject = $this->storage->createAuthCode($code, $client, $data, $redirect_uri, time() + $this->getVariable(self::CONFIG_AUTH_LIFETIME), $scope);
    return $codeObject;
  }
